...
def receive_message(sock_info, request_id, max_message_size=MAX_MESSAGE_SIZE):
    ...
    if request_id is not None:
        if request_id != response_to:
            raise ProtocolError(
                "Got response id %r but expected %r" % (response_to, request_id)
            )
    if length <= 16:
        raise ProtocolError(
            "Message length (%r) not longer than standard message header size (16)"
            % (length,)
        )
    ...
    if op_code == 2012:
        ...
        data = decompress(
            _receive_data_on_socket(sock_info, length - 25, deadline), compressor_id
        )
    else:
    ...
    try:
        ...
        raise ProtocolError(
            "Got opcode %r but expected %r" % (op_code, _UNPACK_REPLY.keys())
        )
