    ...
    def estimated_document_count(
        ...
        def _cmd(session, server, sock_info, read_preference):
            if sock_info.max_wire_version >= 12:
                ...
                cmd = SON(
                    [("aggregate", self.__name), ("pipeline", pipeline), ("cursor", {})]
                )
                cmd.update(kwargs)
                ...
                return self._count_cmd(
                    session, sock_info, read_preference, cmd, collation=None
                )
            ...
            def gen_indexes():
                for index in indexes:
                    if not isinstance(index, IndexModel):
                        raise TypeError(
                            "%r is not an instance of pymongo.operations.IndexModel"
                            % (index,)
                        )
