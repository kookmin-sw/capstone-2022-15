...
def parse_host(entity: str, default_port: Optional[int] = DEFAULT_PORT) -> _Address:
    ...
    if isinstance(port, str):
        if not port.isdigit() or int(port) > 65535 or int(port) <= 0:
            raise ValueError(
                "Port must be an integer between 0 and 65535: %s" % (port,)
            )
        port = int(port)
...
def parse_uri(
    ...
    if uri.startswith(SCHEME):
        ...
        raise InvalidURI(
            "Invalid URI scheme: URI must begin with '%s' or '%s'"
            % (SCHEME, SRV_SCHEME)
        )
    ...
    if is_srv:
        ...
        if len(nodes) != 1:
            raise InvalidURI(
                "%s URIs must include one, and only one, hostname" % (SRV_SCHEME,)
            )
        fqdn, port = nodes[0]
        if port is not None:
