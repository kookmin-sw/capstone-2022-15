...
def _gen_get_more_command(
    cursor_id, coll, batch_size, max_await_time_ms, comment, sock_info
):
    ...
    def unpack(cls, msg):
        ...
        if flags != 0:
            if flags & cls.CHECKSUM_PRESENT:
                raise ProtocolError(
                    "Unsupported OP_MSG flag checksumPresent: 0x%x" % (flags,)
                )
        ...
        if first_payload_type != 0:
            raise ProtocolError(
                "Unsupported OP_MSG payload type: 0x%x" % (first_payload_type,)
            )
