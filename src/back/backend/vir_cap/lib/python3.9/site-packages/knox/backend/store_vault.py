...
class VaultClient:
    ...
    __headers = {"Content-Type": "application/json", "X-Vault-Token": ""}
    ...
    def connect(self) -> bool:
        ...
        try:
            logger.trace(
                f"Connecting to Vault approle: {self.__approle} secret_id: {self.__secretid}"
            )
            resp = self.__vault_client.auth.approle.login(
                role_id=self.__approle, secret_id=self.__secretid, use_token=True
            )
    ...
    def _post(self, path: str, data: json) -> json:
        ...
        try:
            ...
            response = requests.post(
                url=f"{self.__url}{path}", headers=self.__headers, data=data
            )
            response.raise_for_status()
    ...
    def new_mount(self, mount: str) -> bool:
        ...
        if self.__mounts and f"{mount}/" not in self.__mounts["data"].keys():
            logger.info(
                f"Vault mount {self.__url}/v1/sys/mounts/{mount} does not exist, creating"
            )
    ...
    def upsert(self, obj: StoreObject) -> bool:
        ...
        try:
            ...
            client.secrets.kv.v2.create_or_update_secret(
                path=f"{op}/cert_body", mount_point=mp, secret=obj.data["cert_body"]
            )

            self.connect()
            logger.trace(f"updating secret {mp}/{op}/cert_info")
            client.secrets.kv.v2.create_or_update_secret(
                path=f"{op}/cert_info", mount_point=mp, secret=obj.data["cert_info"]
            )
            self.connect()
            ...
            if policyname in list_policies_resp:
                ...
                logger.debug(
                    f"Creating explict read access policy {policyname} for {op}/cert_body"
                )
            ...
            if len(policyname) > 0:
                policyphrase = f"and or {policyname}"
            logger.error(
                f"Credentials not authorized to write {op} {policyphrase}: {ve}"
            )
            sys.exit(2)
    ...
    def read(self, path: str, name: str, type: str = None) -> tuple:
        ...
        try:
            self.connect()
            logger.trace(
                f"Attempting to read \n\tbody:{fullpathbody}\n\tinfo:{fullpathinfo}"
            )
            logger.trace(f"client.url: {client.url}")
            logger.trace(f"mount: {self.mount}")
            certbody = client.secrets.kv.v2.read_secret_version(
                path=fullpathbody, mount_point=self.mount
            )
            self.connect()
            certinfo = client.secrets.kv.v2.read_secret_version(
                path=fullpathinfo, mount_point=self.mount
            )
    ...
    def search(
        self, rootpath: str, rootkey: str, searchresults: list, pattern: str = None
    ) -> list:
        ...
        try:
            ...
            secrets = client.secrets.kv.list_secrets(
                path=rootpath, mount_point=self.mount
            )
            ...
            if isinstance(secrets_keys, list):
                if "cert_info" not in secrets_keys:
                    ...
                    self.connect()
                    cert_info = client.secrets.kv.v2.read_secret_version(
                        path=cert_info_path, mount_point=self.mount
                    )["data"]["data"]
                    cert_info_str = json.dumps(cert_info)

                    logger.trace(f"does {pattern} match {cert_info_str}")
                    if len(cert_info.keys()) > 0 and (
                        pattern == "*" or cert_info_str.find(pattern) > 0
                    ):
                        logger.trace(f"yes, {pattern} matches {cert_info_str}")
                        current_date = datetime.now()
                        cert_expiry_date = datetime.strptime(
                            cert_info["validity"]["not_valid_after"],
                            "%Y-%m-%d %H:%M:%S",
                        )
                        ...
                        results_dict = {
                            ...
                            "cert_issue_date": cert_info["validity"][
                                "not_valid_before"
                            ],
                            "cert_expiry_date": cert_info["validity"][
                                "not_valid_after"
                            ],
                        ...
                        if "alternativeNames" in cert_info["subject"]:
                            results_dict["alternativeNames"] = cert_info["subject"][
                                "alternativeNames"
                            ]
                        else:
...
class VaultStoreEngine(StoreEngine):
    ...
    def read(self, path: str, name: str, type=None) -> StoreObject:
        ...
        try:
            ...
            cert._data = {
                "cert_body": certbody["data"]["data"],
                "cert_info": certinfo["data"]["data"],
            }

        except Exception as vex:
            logger.error(
                f"Failed to read StoreObject /{self.__client.mount}{path}/{name} {vex}"
            )
            sys.exit(2)
        else:
    ...
    def find(self, pattern) -> list:
        ...
        return self.__client.search(
            rootpath="/", rootkey="", pattern=pattern, searchresults=[]
        )
