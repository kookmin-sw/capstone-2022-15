...
class Cert(StoreObject):
    ...
    def __init__(self, settings: LazySettings, common_name=None) -> None:
        ...
        self._type = ""
        super().__init__(
            name=self.name, path=self.path, body=self._body, info=self._info
        )
    ...
    def load_x509(self, path: str) -> None:
        ...
        with open(path, mode="r+", encoding="utf-8") as fp:
            ...
            self._x509 = x509.load_pem_x509_certificate(
                bytes(self._file, "utf-8"), default_backend()
            )
    ...
    def load(
        self, pub: str, key: str, certtype: enum.Enum = PEM, chain: str = None
    ) -> None:
        ...
        if not validators.domain(self.name):
            logger.info(
                f"{self.name} appears to be a client/server certificate not a web/https certificate"
            )
    ...
    def issuer(self) -> str:
        ...
        return json.dumps(
            {attr.oid._name: attr.value for attr in self._x509.issuer}, indent=8
        )
    ...
    def public(self) -> str:
        ...
        if self.type == Cert.PEM.name:
            return (
                self._x509.public_bytes(Encoding.PEM).decode("utf-8").replace("\n", "")
            )
        else:
    ...
    def generate(self) -> None:
        ...
        try:
            ...
            self.load(
                pub=certfile, key=privkey, chain=chainfile, certtype=Cert.PEM.name
            )
        except Exception:
