...
class Store:
    ...
    def __init__(self, settings, engine_name: str = "vault") -> None:
        ...
        try:
            self._engine_name = (
                engine_name if engine_name is not None else settings.STORE_ENGINE
            )
            self._engine = self._engine_map.get(self._engine_name).__call__(settings)
        except Exception:
            logger.error(
                f"StoreEngineFailure KNOX_STORE_ENGINE={self._engine_name} is invalid. Valid options are {self._engine_map.keys()}"
            )  # noqa: E501
            raise
