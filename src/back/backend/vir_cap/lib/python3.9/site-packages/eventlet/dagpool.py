...
class PropagateError(Exception):
    ...
    def __init__(self, key, exc):
        ...
        msg = "PropagateError({0}): {1}: {2}".format(key, exc.__class__.__name__, exc)
...
class DAGPool(object):
    ...
    def spawn(self, key, depends, function, *args, **kwds):
        ...
        newcoro = greenthread.spawn(
            self._wrapper, function, key, self._wait_each(pending), *args, **kwds
        )
    ...
    def items(self):
        ...
        # posted.
        return tuple(
            (key, self._value_or_raise(value))
