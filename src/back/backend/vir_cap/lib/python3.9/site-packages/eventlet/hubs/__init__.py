...
builtin_hub_modules = tuple(
    importlib.import_module("eventlet.hubs." + name) for name in builtin_hub_names
)
...
def use_hub(mod=None):
    ...
    if hasattr(mod, "is_available"):
        if not mod.is_available():
            raise Exception(
                "selected hub is not available on this system mod={}".format(mod)
            )
    else:
...
def trampoline(
    ...
    *timeout* elapses, depending on arguments specified.
    ...
    try:
        if read:
            listener = hub.add(
                hub.READ, fileno, current.switch, current.throw, mark_as_closed
            )
        elif write:
            listener = hub.add(
                hub.WRITE, fileno, current.switch, current.throw, mark_as_closed
            )
        try:
            return hub.switch()
        finally:
