...
DEFAULT_LOG_FORMAT = (
    '%(client_ip)s - - [%(date_time)s] "%(request_line)s"'
...
class Input(object):
    ...
    def set_hundred_continue_response_headers(
        self, headers, capitalize_response_headers=True
    ):
...
def get_logger(log, debug):
    if callable(getattr(log, "info", None)) and callable(getattr(log, "debug", None)):
        return log
    else:
...
class HttpProtocol(BaseHTTPServer.BaseHTTPRequestHandler):
        ...
        try:
            ...
            if hasattr(conn, "send") and hasattr(conn, "recv"):
                ...
                raise NotImplementedError(
                    """eventlet.wsgi doesn't support sockets of type {0}""".format(
                        type(conn)
                    )
                )
        ...
        try:
            ...
            if last_errno in BROKEN_SOCK:
                self.server.log.debug(
                    "({0}) connection reset by peer {1!r}".format(
                        self.server.pid, self.client_address
                    )
                )
    ...
    def handle_one_response(self):
        ...
        def write(data):
            ...
            if not headers_set:
                ...
                if (
                    ...
                    and self.server.keepalive
                    and (
                        client_conn == "keep-alive"
                        or (
                            self.request_version == "HTTP/1.1"
                            and not client_conn == "close"
                        )
                    )
                ):
        ...
        try:
            try:
                ...
                if headers_set and not headers_sent and hasattr(result, "__len__"):
                    ...
                    if "Content-Length" not in [h for h, _v in headers_set[1]]:
                        headers_set[1].append(
                            ("Content-Length", str(sum(map(len, result))))
                        )
            ...
            if request_input.chunked_input or request_input.position < (
                request_input.content_length or 0
            ):
                    ...
                    try:
                        ...
                        self.server.log.error(
                            (
                                ...
                                + ' client={0} request="{1}" error="{2}"'
                            ).format(
                                self.get_client_address()[0],
                                self.requestline,
                                e,
                            )
                        )
    ...
    def get_environ(self):
        ...
        env["headers_raw"] = headers_raw = tuple(
...
If unsure, use eventlet.GreenPool."""
    ...
    try:
        serv.log.info(
            "({0}) wsgi starting up on {1}".format(serv.pid, socket_repr(sock))
        )
        while is_accepting:
            try:
                ...
                (
                    pool.spawn(serv.process_request, connection).link(
                        _clean_connection, connection
                    )
                )
        ...
        pool.waitall()
        serv.log.info(
            "({0}) wsgi exited, is_accepting={1}".format(serv.pid, is_accepting)
        )
        try:
