...
class WebSocketWSGI(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if not (
            "upgrade" in http_connection_parts
        ...
        for ext in header.split(","):
            ...
            for part in parts[1:]:
                ...
                if len(key_val) == 1:
                    ...
                    config[key_val[0].strip().lower()] = (
                        key_val[1].strip().strip('"').lower()
                    )
    ...
    def _negotiate_permessage_deflate(self, extensions):
        ...
        for config in deflate:
            ...
            want_config = {
                ...
                "server_no_context_takeover": config.get(
                    "server_no_context_takeover", False
                ),
                "client_no_context_takeover": config.get(
            ...
            if mwb is not None:
                if mwb is True:
                    ...
                    want_config["server_max_window_bits"] = int(
                        config.get("server_max_window_bits", max_wbits)
                    )
            ...
            if mwb is not None:
                if mwb is True:
                    ...
                    want_config["client_max_window_bits"] = int(
                        config.get("client_max_window_bits", max_wbits)
                    )
    ...
    def _handle_hybi_request(self, environ):
        ...
        if negotiated_protocol:
            handshake_reply.append(
                b"Sec-WebSocket-Protocol: " + six.b(negotiated_protocol)
            )
...
class RFC6455WebSocket(WebSocket):
        ...
        def getvalue(self):
            ...
            if not self.opcode & 8 and self.decompressor:
                data = self.decompressor.decompress(
                    data + b"\x00\x00\xff\xff", self.max_frame_length
                )
                if self.decompressor.unconsumed_tail:
                    raise FailedConnectionError(
                        1009,
                        "Incoming compressed frame exceeds length limit of {} bytes.".format(
    ...
    def _apply_mask(data, mask, length=None, offset=0):
        ...
        cnt = range(length)
        return b"".join(
            six.int2byte(six.indexbytes(data, i) ^ mask[(offset + i) % 4]) for i in cnt
        )
        ...
        if opcode == 8:  # connection close
            ...
            if not data:
                ...
                try:
                    ...
                    raise FailedConnectionError(
                        1002, "Close message data should be valid UTF-8."
                    )
            else:
                status = 1002
    ...
    def _recv_frame(self, message=None):
        ...
        if not masked and not self.client:
            raise FailedConnectionError(
                1002, "A client MUST mask all frames" " that it sends to the server"
            )
        length = b & 127
        if opcode & 8:
            if not finished:
                raise FailedConnectionError(
                    1002, "Control frames must not" " be fragmented."
                )
            if length > 125:
                raise FailedConnectionError(
                    1002,
            ...
            raise FailedConnectionError(
                1002, "Received a non-continuation opcode within" " fragmented message."
            )
        elif not opcode and not message:
            raise FailedConnectionError(
                1002,
                "Received continuation opcode with no previous" " fragments received.",
            )
        if length == 126:
        ...
        if length > self.max_frame_length:
            raise FailedConnectionError(
                1009,
                "Incoming frame of {} bytes is above length limit of {} bytes.".format(
    ...
    def _pack_message(
        self, message, masked=False, continuation=False, final=True, control_code=None
    ):
        is_text = False
        if isinstance(message, six.text_type):
            message = message.encode("utf-8")
