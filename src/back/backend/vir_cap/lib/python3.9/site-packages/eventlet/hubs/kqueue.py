...
class Hub(hub.BaseHub):
    ...
    def _reinit_kqueue(self):
        self.kqueue.close()
        self._init_kqueue()
        events = [e for i in six.itervalues(self._events) for e in six.itervalues(i)]
    ...
    def add(self, evtype, fileno, cb, tb, mac):
        ...
        if evtype not in events:
            try:
                event = select.kevent(
                    fileno, self.FILTERS.get(evtype), select.KQ_EV_ADD
                )
    ...
    def _delete_events(self, events):
        del_events = [
            select.kevent(e.ident, e.filter, select.KQ_EV_DELETE) for e in events
        ]
