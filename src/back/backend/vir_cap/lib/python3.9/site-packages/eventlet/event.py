...
class Event(object):
        ...
        if self._result is NOT_USED:
            ...
            if timeout is not None:
                timer = hub.schedule_call_local(
                    timeout, self._do_send, None, None, current
                )
            try:
                result = hub.switch()
    ...
    def send(self, result=None, exc=None):
        ...
        """
        assert (
            self._result is NOT_USED
        ), "Trying to re-send() an already-triggered event."
        self._result = result
        ...
        for waiter in self._waiters:
            hub.schedule_call_global(0, self._do_send, self._result, self._exc, waiter)
