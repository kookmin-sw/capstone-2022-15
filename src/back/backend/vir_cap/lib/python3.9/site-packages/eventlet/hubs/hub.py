...
def alarm_handler(signum, frame):
    ...
    raise RuntimeError(
        "Blocking detector ALARMED at" + str(inspect.getframeinfo(frame))
    )
...
class BaseHub(object):
        ...
        if fileno in bucket:
            if g_prevent_multiple_readers:
                raise RuntimeError(
                    ...
                    "THAT THREAD=%s" % (evtype, fileno, evtype, cb, bucket[fileno])
                )
    ...
    def close_one(self):
        ...
        if not listener.greenlet.dead:
            ...
            listener.tb(
                eventlet.hubs.IOClosed(errno.ENOTCONN, "Operation on closed file")
            )
    ...
    def abort(self, wait=False):
        ...
        if wait:
            assert (
                self.greenlet is not greenlet.getcurrent()
