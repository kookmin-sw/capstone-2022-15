...
class _QueueLock(object):
    ...
    def acquire(self):
        ...
        if (self._waiters or self._count > 0) and self._holder is not current:
            ...
            assert (
                self._count == 0
            ), "After waking a thread, the lock must be unacquired"
...
class _BlockedThread(object):
    ...
    def wake(self):
        ...
        if self._blocked_thread is not None and self._wakeupper is None:
            self._wakeupper = self._hub.schedule_call_global(
                0, self._blocked_thread.switch
            )
            return True
        return False
...
class Socket(_Socket):
    ...
    def __init__(self, context, socket_type):
        ...
        self.__dict__["_eventlet_listener"] = hub.add(
            hub.READ, self.getsockopt(FD), event, lambda _: None, lambda: None
        )
