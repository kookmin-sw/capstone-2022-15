...
if _B:
    ...
    from .compat import StreamType, StreamTextType, VersionType
...
class YAML:
    def __init__(self, _kw=enforce, typ=_A, pure=_B, output=_A, plug_ins=_A):
        if _kw is not enforce:
            raise TypeError(_H.format(self.__class__.__name__, _kw))
        self.typ = [_E] if typ is _A else typ if isinstance(typ, list) else [typ]
        self.pure = pure
        self._output = output
        self._context_manager = _A
        self.plug_ins = []
        for pu in ([] if plug_ins is _A else plug_ins) + self.official_plug_ins():
        ...
        self.allow_unicode = _C
        self.Reader = _A
        self.Representer = _A
        self.Constructor = _A
        self.Scanner = _A
        self.Serializer = _A
        self.default_flow_style = _A
        typ_found = 1
        setup_rt = _B
        if _E in self.typ:
            setup_rt = _C
            ...
            self.Emitter = (
                ruamel.yaml.emitter.Emitter if pure or CEmitter is _A else CEmitter
            )
            self.Representer = ruamel.yaml.representer.SafeRepresenter
            self.Parser = (
                ruamel.yaml.parser.Parser if pure or CParser is _A else CParser
            )
        ...
        self.line_break = _A
        self.map_indent = _A
        self.sequence_indent = _A
        self.sequence_dash_offset = 0
        self.compact_seq_seq = _A
        self.compact_seq_map = _A
        self.sort_base_mapping_type_on_output = _A
        self.top_level_colon_align = _A
        self.prefix_colon = _A
        self.version = _A
        self.preserve_quotes = _A
        self.allow_duplicate_keys = _B
        self.encoding = _J
        self.explicit_start = _A
        self.explicit_end = _A
        self.tags = _A
        self.default_style = _A
        self.top_level_block_style_scalar_no_indent_error_1_1 = _B
        self.scalar_after_indicator = _A
        self.brace_single_entry_mapping_in_flow_sequence = _B
        for module in self.plug_ins:
            if getattr(module, _K, _A) in self.typ:
                typ_found += 1
    ...
    def reader(self):
        try:
            ...
            self._reader = self.Reader(_A, loader=self)
            return self._reader
    ...
    def representer(self):
        ...
        if not hasattr(self, attr):
            ...
            if self.sort_base_mapping_type_on_output is not _A:
    ...
    def get_constructor_parser(self, stream):
        if self.Parser is not CParser:
            if self.Reader is _A:
                self.Reader = ruamel.yaml.reader.Reader
            if self.Scanner is _A:
            ...
            class XLoader(self.Parser, self.Constructor, rslvr):
                def __init__(selfx, stream, version=self.version, preserve_quotes=_A):
    ...
    def dump(self, data, stream=_A, _kw=enforce, transform=_A):
        if self._context_manager:
            ...
            if _kw is not enforce:
                raise TypeError(
                    "{}.dump() takes one positional argument but at least two were given ({!r})".format(
                        self.__class__.__name__, _kw
                    )
                )
            if transform is not _A:
                raise TypeError(
                    "{}.dump() in the context manager cannot have transform keyword ".format(
    ...
    def dump_all(self, documents, stream, _kw=enforce, transform=_A):
        if self._context_manager:
            raise NotImplementedError
        if _kw is not enforce:
            raise TypeError(_N.format(self.__class__.__name__, _kw))
        self._output = stream
    ...
    def _indent(self, mapping=_A, sequence=_A, offset=_A):
        if mapping is not _A:
            self.map_indent = mapping
        if sequence is not _A:
    ...
    def compact(self, seq_seq=_A, seq_map=_A):
...
class YAMLContextManager:
    def __init__(self, yaml, transform=_A):
        self._yaml = yaml
        self._output_inited = _B
        self._output_path = _A
        self._output = self._yaml._output
        self._transform = transform
        if not hasattr(self._output, _O) and hasattr(self._output, _F):
    ...
    def teardown_output(self):
        ...
        try:
            ...
            raise
        try:
            delattr(self._yaml, _P)
            delattr(self._yaml, _Q)
        except AttributeError:
            raise
        if self._transform:
...
def safe_load(stream, version=_A):
    return load(stream, SafeLoader, version)


def safe_load_all(stream, version=_A):
    return load_all(stream, SafeLoader, version)


def round_trip_load(stream, version=_A, preserve_quotes=_A):
    return load(stream, RoundTripLoader, version, preserve_quotes=preserve_quotes)


def round_trip_load_all(stream, version=_A, preserve_quotes=_A):
...
def serialize(node, stream=_A, Dumper=Dumper, **kwds):
...
def dump_all(
    ...
    getvalue = _A
    if top_level_colon_align is _C:
...
def safe_dump_all(documents, stream=_A, **kwds):
    return dump_all(documents, stream, Dumper=SafeDumper, **kwds)


def safe_dump(data, stream=_A, **kwds):
...
def round_trip_dump(
    ...
    allow_unicode = _C if allow_unicode is _A else allow_unicode
    return dump_all(
        [data],
        stream,
        Dumper=Dumper,
...
def add_implicit_resolver(
    tag, regexp, first=_A, Loader=_A, Dumper=_A, resolver=Resolver
):
    A = "add_implicit_resolver"
    if Loader is _A and Dumper is _A:
...
def add_multi_constructor(
    tag_prefix, multi_constructor, Loader=_A, constructor=Constructor
):
    if Loader is _A:
        constructor.add_multi_constructor(tag_prefix, multi_constructor)
    else:
        if _B and hasattr(Loader, "add_multi_constructor"):
...
def add_multi_representer(
    data_type, multi_representer, Dumper=_A, representer=Representer
):
    if Dumper is _A:
...
class YAMLObjectMetaclass(type):
    def __init__(cls, name, bases, kwds):
        ...
        if _G in kwds and kwds[_G] is not _A:
