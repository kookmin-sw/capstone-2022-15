...
def _exists(filename, create=_E):
    ...
    if create:
        try:
            ...
            raise BoxError(f"Could not create file {A} - {C}")
        else:
            return
    if not B.exists():
        raise BoxError(f'File "{A}" does not exist')
    if not B.is_file():
        raise BoxError(f"{A} is not a file")


def _to_json(obj, filename=_A, encoding=_B, errors=_C, **C):
    A = filename
    B = json.dumps(obj, ensure_ascii=_E, **C)
    if A:
        _exists(A, create=_D)
        with open(A, _F, encoding=encoding, errors=errors) as D:
            D.write(B if sys.version_info >= (3, 0) else B.decode(_B))
    else:
        return B
...
def _to_yaml(obj, filename=_A, default_flow_style=_E, encoding=_B, errors=_C, **C):
    B = default_flow_style
    A = filename
    if A:
        _exists(A, create=_D)
        with open(A, _F, encoding=encoding, errors=errors) as D:
            yaml.dump(obj, stream=D, default_flow_style=B, **C)
    else:
        return yaml.dump(obj, default_flow_style=B, **C)
...
def _from_yaml(yaml_string=_A, filename=_A, encoding=_B, errors=_C, **A):
    ...
    if B:
        _exists(B)
        with open(B, _G, encoding=encoding, errors=errors) as E:
            D = yaml.load(E, **A)
    elif C:
        D = yaml.load(C, **A)
    else:
...
def _to_toml(obj, filename=_A, encoding=_B, errors=_C):
    ...
    if A:
        _exists(A, create=_D)
        with open(A, _F, encoding=encoding, errors=errors) as B:
            toml.dump(obj, B)
    else:
...
def _from_toml(toml_string=_A, filename=_A, encoding=_B, errors=_C):
    ...
    if A:
        _exists(A)
        with open(A, _G, encoding=encoding, errors=errors) as D:
            C = toml.load(D)
    elif B:
        C = toml.loads(B)
    else:
...
def _to_csv(box_list, filename, encoding=_B, errors=_C):
    ...
    if B:
        ...
        with open(B, _F, encoding=encoding, errors=errors, newline="") as F:
            D = csv.DictWriter(F, fieldnames=C)
            D.writeheader()
            for G in A:
                D.writerow(G)
...
def _from_csv(filename, encoding=_B, errors=_C):
    A = filename
    _exists(A)
    with open(A, _G, encoding=encoding, errors=errors, newline="") as B:
        C = csv.DictReader(B)
        return [A for A in C]
