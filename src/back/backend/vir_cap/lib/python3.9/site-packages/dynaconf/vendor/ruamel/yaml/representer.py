...
if _C:
    from typing import Dict, List, Any, Union, Text, Optional
__all__ = [
    "BaseRepresenter",
    "SafeRepresenter",
    "Representer",
...
class BaseRepresenter:
    ...
    def __init__(self, default_style=_A, default_flow_style=_A, dumper=_A):
        ...
        self.object_keeper = []
        self.alias_key = _A
        self.sort_base_mapping_type_on_output = _B

    @property
    def serializer(self):
        try:
            if hasattr(self.dumper, _S):
    ...
    def represent_data(self, data):
        ...
        if data_types[0] in self.yaml_representers:
            ...
            for data_type in data_types:
                ...
                if _A in self.yaml_multi_representers:
                    node = self.yaml_multi_representers[_A](self, data)
                elif _A in self.yaml_representers:
                    node = self.yaml_representers[_A](self, data)
                else:
                    node = ScalarNode(_A, text_type(data))
        return node
    ...
    def represent_sequence(self, tag, sequence, flow_style=_A):
        ...
        if flow_style is _A:
            if self.default_flow_style is not _A:
...
class SafeRepresenter(BaseRepresenter):
    def ignore_aliases(self, data):
        if data is _A or isinstance(data, tuple) and data == ():
            return _B
    ...
    def represent_none(self, data):
        return self.represent_scalar(_L, _T)

    if PY3:

        def represent_str(self, data):
            return self.represent_scalar(_D, data)

        def represent_binary(self, data):
            if hasattr(base64, "encodebytes"):
                data = base64.encodebytes(data).decode(_E)
            else:
                data = base64.encodestring(data).decode(_E)
        ...
        def represent_str(self, data):
            ...
            style = _A
            try:
                data = unicode(data, _E)
                tag = _D
            except UnicodeDecodeError:
                try:
                    data = unicode(data, _U)
                    tag = _D
                except UnicodeDecodeError:
                    data = data.encode(_V)
                    tag = _M
    ...
    def represent_int(self, data):
        return self.represent_scalar(_G, text_type(data))
    ...
    def represent_float(self, data):
        if data != data or data == _K and data == 1.0:
            value = _W
            ...
            if getattr(self.serializer, "use_version", _A) == (1, 1):
                if _H not in value and "e" in value:
                    value = value.replace("e", ".0e", 1)
        return self.represent_scalar(_N, value)

    def represent_list(self, data):
        return self.represent_sequence(_O, data)

    def represent_dict(self, data):
        return self.represent_mapping(_P, data)
    ...
    def represent_date(self, data):
        ...
        return self.represent_scalar(_Q, value)
...
class Representer(SafeRepresenter):
    ...
    def represent_name(self, data):
        try:
            name = _I % (data.__module__, data.__qualname__)
        except AttributeError:
            name = _I % (data.__module__, data.__name__)
    ...
    if PY2:
        ...
        def represent_instance(self, data):
            cls = data.__class__
            class_name = _I % (cls.__module__, cls.__name__)
            args = _A
            state = _A
            ...
            if hasattr(data, _a):
                ...
                state = data.__dict__
            if args is _A and isinstance(state, dict):
                return self.represent_mapping(_b + class_name, state)
            if isinstance(state, dict) and not state:
                return self.represent_sequence(_R + class_name, args)
            value = {}
            if bool(args):
                value[_c] = args
            value[_d] = state
            return self.represent_mapping(_R + class_name, value)
    ...
    def represent_object(self, data):
        ...
        reduce = (list(reduce) + [_A] * 5)[:5]
        function, args, state, listitems, dictitems = reduce
        args = list(args)
        if state is _A:
            state = {}
        if listitems is not _A:
            listitems = list(listitems)
        if dictitems is not _A:
        ...
        if function.__name__ == "__newobj__":
            ...
            newobj = _C
        try:
            function_name = _I % (function.__module__, function.__qualname__)
        except AttributeError:
            function_name = _I % (function.__module__, function.__name__)
        if (
            not args
            and not listitems
            and not dictitems
            ...
            return self.represent_mapping(_b + function_name, state)
...
class RoundTripRepresenter(SafeRepresenter):
    def __init__(self, default_style=_A, default_flow_style=_A, dumper=_A):
        if not hasattr(dumper, _S) and default_flow_style is _A:
            default_flow_style = _C
    ...
    def ignore_aliases(self, data):
        try:
            if data.anchor is not _A and data.anchor.value is not _A:
                return _C
        except AttributeError:
            pass
    ...
    def represent_literal_scalarstring(self, data):
        tag = _A
        style = _J
        anchor = data.yaml_anchor(any=_B)
    ...
    def represent_scalar_float(self, data):
        ...
        if data._exp is _A and data._prec > 0 and data._prec == data._width - 1:
            value = "{}{:d}.".format(
            ...
            if prec == 0 or prec == 1 and ms != "":
                value = value.replace("0.", _H)
            while len(value) < data._width:
                value += A
        else:
            m, es = "{:{}.{}e}".format(
                data, data._width, data._width + (1 if data._m_sign else 0)
            ).split("e")
            ...
            if data._prec < 0:
                ...
                e -= len(m2)
                value = m1 + m2 + _H + data._exp + B.format(e, esgn, data._e_width)
            else:
                if data._m_lead0 > 0:
                    m2 = A * (data._m_lead0 - 1) + m1 + m2
                    m1 = A
                    m2 = m2[: -data._m_lead0]
                    e += data._m_lead0
    ...
    def represent_sequence(self, tag, sequence, flow_style=_A):
        ...
        for (idx, item) in enumerate(sequence):
            ...
            value.append(node_item)
        if flow_style is _A:
            if len(sequence) != 0 and self.default_flow_style is not _A:
                node.flow_style = self.default_flow_style
            else:
                node.flow_style = best_style
        return node
    ...
    def represent_key(self, data):
        if isinstance(data, CommentedKeySeq):
            self.alias_key = _A
            return self.represent_sequence(_O, data, flow_style=_B)
        if isinstance(data, CommentedKeyMap):
            self.alias_key = _A
            return self.represent_mapping(_P, data, flow_style=_B)
    ...
    def represent_mapping(self, tag, mapping, flow_style=_A):
        ...
        for (item_key, item_value) in items:
            ...
            if item_comment:
                assert getattr(node_key, _F, _A) is _A
                node_key.comment = item_comment[:2]
                nvc = getattr(node_value, _F, _A)
                if nvc is not _A:
        ...
        if flow_style is _A:
            ...
            ) and self.default_flow_style is not _A:
                node.flow_style = self.default_flow_style
            else:
                node.flow_style = best_style
        if bool(merge_list):
    ...
    def represent_omap(self, tag, omap, flow_style=_A):
        ...
        for item_key in omap:
            ...
            if item_comment:
                if item_comment[1]:
                    node_item.comment = [_A, item_comment[1]]
                assert getattr(node_item.value[0][0], _F, _A) is _A
                node_item.value[0][0].comment = [item_comment[0], _A]
                nvc = getattr(node_item.value[0][1], _F, _A)
                if nvc is not _A:
                    nvc[0] = item_comment[2]
                    nvc[1] = item_comment[3]
                else:
    ...
    def represent_set(self, setting):
        ...
        for item_key in setting.odict:
            node_key = self.represent_key(item_key)
            node_value = self.represent_data(_A)
            item_comment = item_comments.get(item_key)
            if item_comment:
                assert getattr(node_key, _F, _A) is _A
                node_key.comment = item_comment[:2]
