...
class Validator:
    ...
    def validate(
        ...
        if len(self.envs) == 1 and self.envs[0].upper() == settings.current_env.upper():
        ...
        for env in self.envs:
            self._validate_items(settings.from_env(env), only=only, exclude=exclude)
    ...
    def _validate_items(
        ...
        for name in self.names:
            ...
            if self.must_exist is False and value is not empty:
                raise ValidationError(
                    self.messages["must_exist_false"].format(name=name, env=env)
                )
...
class ValidatorList(list):
    ...
    def descriptions(self, flat: bool = False) -> Dict[str, Union[str, List[str]]]:
        ...
        for validator in self:
            for name in validator.names:
                ...
                if flat:
                    ...
                    descriptions[name].append(validator.description)  # type: ignore
        return descriptions
