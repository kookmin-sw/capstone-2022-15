...
class LazySettings(LazyObject):
    ...
    def _setup(self):
        ...
        self._wrapped = Settings(settings_module=settings_module, **self._kwargs)
...
class Settings:
    ...
    def get(
        ...
        if (
            fresh or self._fresh or key in getattr(self, "FRESH_VARS_FOR_DYNACONF", ())
    ...
    def _merge_before_set(self, existing, value):
        ...
        if isinstance(value, (list, tuple)):
            local_merge = "dynaconf_merge" in value or "dynaconf_merge_unique" in value
            if global_merge or local_merge:
                value = list(value)
                unique = False
                if local_merge:
