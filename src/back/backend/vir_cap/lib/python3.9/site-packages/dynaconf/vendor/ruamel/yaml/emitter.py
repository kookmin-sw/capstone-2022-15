...
if _C:
    from typing import Any, Dict, List, Union, Text, Tuple, Optional
...
class Indents:
    ...
    def seq_flow_align(self, seq_indent, column):
        if len(self.values) < 2 or not self.values[-1][1]:
            return 0
        base = self.values[-1][0] if self.values[-1][0] is not _B else 0
...
class Emitter:
    ...
    def __init__(
        ...
        line_break=_B,
        block_seq_indent=_B,
        top_level_colon_align=_B,
        prefix_colon=_B,
        brace_single_entry_mapping_in_flow_sequence=_B,
        dumper=_B,
    ):
        self.dumper = dumper
        if self.dumper is not _B and getattr(self.dumper, "_emitter", _B) is _B:
            self.dumper._emitter = self
        self.stream = stream
        self.encoding = _B
        self.allow_space_break = _B
        self.states = []
        ...
        self.indent = _B
        self.flow_context = []
        self.root_context = _C
        self.sequence_context = _C
        self.mapping_context = _C
        self.simple_key_context = _C
        self.line = 0
        self.column = 0
        self.whitespace = _A
        self.indention = _A
        self.compact_seq_seq = _A
        self.compact_seq_map = _A
        self.no_newline = _B
        self.open_ended = _C
        self.colon = ":"
        self.prefixed_colon = (
            self.colon if prefix_colon is _B else prefix_colon + self.colon
        )
        ...
        self.unicode_supplementary = sys.maxunicode > 65535
        ...
        self.tag_prefixes = _B
        self.prepared_anchor = _B
        self.prepared_tag = _B
        self.analysis = _B
        self.style = _B
        self.scalar_after_indicator = _A
    ...
    def expect_stream_start(self):
        ...
        if isinstance(self.event, StreamStartEvent):
            if PY2:
                if self.event.encoding and not getattr(self.stream, A, _B):
                    self.encoding = self.event.encoding
            elif self.event.encoding and not hasattr(self.stream, A):
                self.encoding = self.event.encoding
    ...
    def expect_first_document_start(self):
        return self.expect_document_start(first=_A)

    def expect_document_start(self, first=_C):
        if isinstance(self.event, DocumentStartEvent):
            if (self.event.version or self.event.tags) and self.open_ended:
                self.write_indicator(_M, _A)
                self.write_indent()
    ...
    def expect_alias(self):
        if self.event.anchor is _B:
            raise EmitterError("anchor is not specified for alias")
        self.process_anchor("*")
        self.state = self.states.pop()

    def expect_scalar(self):
        self.increase_indent(flow=_A)
        self.process_scalar()
    ...
    def expect_flow_sequence(self):
        ...
        self.write_indicator(_D * ind + _K, _A, whitespace=_A)
        self.increase_indent(flow=_A, sequence=_A)
        self.flow_context.append(_K)
    ...
    def expect_flow_mapping(self, single=_C):
        ...
        if (
            single
            and self.flow_level
            and self.flow_context[-1] == _K
            and not self.canonical
            and not self.brace_single_entry_mapping_in_flow_sequence
        ):
            map_init = ""
        self.write_indicator(_D * ind + map_init, _A, whitespace=_A)
        self.flow_context.append(map_init)
        self.increase_indent(flow=_A, sequence=_C)
    ...
    def expect_first_flow_mapping_key(self):
        if isinstance(self.event, MappingEndEvent):
            ...
            if not self.canonical and self.check_simple_key():
                ...
                self.expect_node(mapping=_A, simple_key=_A)
            else:
                self.write_indicator(_I, _A)
                self.states.append(self.expect_flow_mapping_value)
                self.expect_node(mapping=_A)
    ...
    def expect_flow_mapping_simple_value(self):
        self.write_indicator(self.prefixed_colon, _C)
    ...
    def expect_flow_mapping_value(self):
        if self.canonical or self.column > self.best_width:
            self.write_indent()
        self.write_indicator(self.prefixed_colon, _A)
        self.states.append(self.expect_flow_mapping_key)
        self.expect_node(mapping=_A)
    ...
    def expect_block_sequence(self):
        if self.mapping_context:
            ...
            if not self.compact_seq_seq and self.column != 0:
                self.write_line_break()
        self.increase_indent(flow=_C, sequence=_A, indentless=indentless)
        self.state = self.expect_first_block_sequence_item

    def expect_first_block_sequence_item(self):
        return self.expect_block_sequence_item(first=_A)
    ...
    def expect_block_sequence_item(self, first=_C):
        if not first and isinstance(self.event, SequenceEndEvent):
            ...
            if self.event.comment and self.event.comment[1]:
                self.write_pre_comment(self.event)
            nonl = self.no_newline if self.column == 0 else _C
            self.write_indent()
            ind = self.sequence_dash_offset
            self.write_indicator(_D * ind + "-", _A, indention=_A)
    ...
    def expect_block_mapping(self):
        if not self.mapping_context and not (self.compact_seq_map or self.column == 0):
            self.write_line_break()
        self.increase_indent(flow=_C, sequence=_C)
        self.state = self.expect_first_block_mapping_key

    def expect_first_block_mapping_key(self):
        return self.expect_block_mapping_key(first=_A)
        ...
        if not first and isinstance(self.event, MappingEndEvent):
            ...
            if self.check_simple_key():
                if not isinstance(self.event, (SequenceStartEvent, MappingStartEvent)):
                    try:
                        if self.event.style == _I:
                            self.write_indicator(_I, _A, indention=_A)
                    except AttributeError:
                        pass
                self.states.append(self.expect_block_mapping_simple_value)
                self.expect_node(mapping=_A, simple_key=_A)
                if isinstance(self.event, AliasEvent):
                    self.stream.write(_D)
            else:
                self.write_indicator(_I, _A, indention=_A)
    ...
    def expect_block_mapping_simple_value(self):
        if getattr(self.event, "style", _B) != _I:
            if self.indent == 0 and self.top_level_colon_align is not _B:
                c = _D * (self.top_level_colon_align - self.column) + self.colon
            else:
    ...
    def expect_block_mapping_value(self):
        self.write_indent()
        self.write_indicator(self.prefixed_colon, _A, indention=_A)
        self.states.append(self.expect_block_mapping_key)
        self.expect_node(mapping=_A)
    ...
    def check_simple_key(self):
        ...
        if isinstance(self.event, NodeEvent) and self.event.anchor is not _B:
            if self.prepared_anchor is _B:
                self.prepared_anchor = self.prepare_anchor(self.event.anchor)
            length += len(self.prepared_anchor)
        if (
            isinstance(self.event, (ScalarEvent, CollectionStartEvent))
            and self.event.tag is not _B
        ):
            if self.prepared_tag is _B:
                self.prepared_tag = self.prepare_tag(self.event.tag)
            length += len(self.prepared_tag)
        if isinstance(self.event, ScalarEvent):
            if self.analysis is _B:
        ...
        return length < self.MAX_SIMPLE_KEY_LENGTH and (
            ...
            and self.event.flow_style is _A
            or isinstance(self.event, MappingStartEvent)
            and self.event.flow_style is _A
    ...
    def process_anchor(self, indicator):
        ...
        if self.prepared_anchor:
            self.write_indicator(indicator + self.prepared_anchor, _A)
            self.no_newline = _C
        self.prepared_anchor = _B
        return _A
    ...
    def process_tag(self):
        ...
        if isinstance(self.event, ScalarEvent):
            ...
            if (not self.canonical or tag is _B) and (
                self.style == ""
                ...
                self.prepared_tag = _B
                return
            if self.event.implicit[0] and tag is _B:
                tag = _G
                self.prepared_tag = _B
        elif (not self.canonical or tag is _B) and self.event.implicit:
            self.prepared_tag = _B
            return
        if tag is _B:
    ...
    def choose_scalar_style(self):
        if self.analysis is _B:
            self.analysis = self.analyze_scalar(self.event.value)
        if self.event.style == _H or self.canonical:
            return _H
        if (not self.event.style or self.event.style == _I) and (
        ...
        if not self.event.style and self.analysis.allow_double_quoted:
            if _J in self.event.value or _E in self.event.value:
                return _H
    ...
    def prepare_tag_handle(self, handle):
        ...
        if handle[0] != _G or handle[-1] != _G:
            raise EmitterError(
                "tag handle must start and end with '!': %r" % utf8(handle)
            )
        for ch in handle[1:-1]:
            if not (
                _O <= ch <= "9" or "A" <= ch <= "Z" or _P <= ch <= "z" or ch in "-_"
            ):
                raise EmitterError(
    ...
    def prepare_tag_prefix(self, prefix):
        ...
        if self.dumper:
            ...
            if version is _B or version >= (1, 2):
                ch_set += "#"
        while end < len(prefix):
            ch = prefix[end]
            if _O <= ch <= "9" or "A" <= ch <= "Z" or _P <= ch <= "z" or ch in ch_set:
                end += 1
            else:
                if start < end:
    ...
    def prepare_tag(self, tag):
        ...
        for prefix in prefixes:
            if tag.startswith(prefix) and (prefix == _G or len(prefix) < len(tag)):
    ...
    def analyze_scalar(self, scalar):
        ...
        if not scalar:
            return ScalarAnalysis(
                ...
                multiline=_C,
                allow_flow_plain=_C,
                allow_block_plain=_A,
                allow_single_quoted=_A,
                allow_double_quoted=_A,
                allow_block=_C,
            )
        block_indicators = _C
        flow_indicators = _C
        line_breaks = _C
        special_characters = _C
        leading_space = _C
        leading_break = _C
        trailing_space = _C
        trailing_break = _C
        break_space = _C
        space_break = _C
        if scalar.startswith(_S) or scalar.startswith(_M):
            block_indicators = _A
            flow_indicators = _A
        preceeded_by_whitespace = _A
        followed_by_whitespace = len(scalar) == 1 or scalar[1] in A
        previous_space = _C
        previous_break = _C
        index = 0
        while index < len(scalar):
            ...
            if index == 0:
                ...
                if ch in ",[]{}":
                    flow_indicators = _A
                if ch == _I and self.serializer.use_version == (1, 1):
                    flow_indicators = _A
                if ch == ":":
                    if followed_by_whitespace:
                        flow_indicators = _A
            ...
            if not (ch == _E or _D <= ch <= "~"):
                if (
                    ...
                    or _Q <= ch <= _X
                    or _Y <= ch <= "�"
                    or self.unicode_supplementary
                    and "𐀀" <= ch <= "\U0010ffff"
                ) and ch != _Z:
                    if not self.allow_unicode:
                        special_characters = _A
                else:
                    special_characters = _A
            if ch == _D:
                if index == 0:
                ...
                previous_break = _C
            index += 1
            preceeded_by_whitespace = ch in A
            followed_by_whitespace = index + 1 >= len(scalar) or scalar[index + 1] in A
        allow_flow_plain = _A
        allow_block_plain = _A
        allow_single_quoted = _A
        allow_double_quoted = _A
        ...
        if leading_space or leading_break or trailing_space or trailing_break:
            allow_flow_plain = allow_block_plain = _C
        if trailing_space:
            allow_block = _C
        if break_space:
        ...
        if special_characters:
            allow_flow_plain = (
                allow_block_plain
            ) = allow_single_quoted = allow_block = _C
        elif space_break:
            allow_flow_plain = allow_block_plain = allow_single_quoted = _C
    ...
    def write_stream_start(self):
        if self.encoding and self.encoding.startswith("utf-16"):
            self.stream.write(_Z.encode(self.encoding))
            ...
            if (
                ...
                if ch is not _B:
                    if ch in self.ESCAPE_REPLACEMENTS:
                        data = _L + self.ESCAPE_REPLACEMENTS[ch]
                    elif ch <= "ÿ":
                        data = "\\x%02X" % ord(ch)
                    elif ch <= "\uffff":
                        data = "\\u%04X" % ord(ch)
    ...
    def determine_block_hints(self, text):
        ...
        if text:
            ...
            if text[-1] not in _F:
                indicator = "-"
            elif len(text) == 1 or text[-2] in _F:
                indicator = "+"
        hints += indicator
        ...
        while end <= len(text):
            ...
            if breaks:
                if ch is _B or ch not in _F:
                    ...
                    if ch is not _B:
                        if self.root_context:
                            idnx = self.indent if self.indent is not _B else 0
                            self.stream.write(_D * (_indent + idnx))
                        else:
                            self.write_indent()
                    start = end
    ...
    def write_comment(self, comment, pre=_C):
        value = comment.value
        if not pre and value[-1] == _E:
            value = value[:-1]
        try:
            col = comment.start_mark.column
            if comment.value and comment.value.startswith(_E):
                col = self.column
        ...
        try:
            ...
            if self.column and value.strip() and nr_spaces < 1 and value[0] != _E:
                nr_spaces = 1
    ...
    def write_pre_comment(self, event):
        ...
        try:
            ...
            for comment in comments:
                if isinstance(event, start_events) and getattr(comment, "pre_done", _B):
                    continue
                if self.column != 0:
                    self.write_line_break()
