...
class Parser:
    ...
    def __init__(self, loader):
        self.loader = loader
        if self.loader is not _A and getattr(self.loader, "_parser", _A) is _A:
            self.loader._parser = self
        self.reset_parser()

    def reset_parser(self):
        self.current_event = _A
        self.tag_handles = {}
        self.states = []
        self.marks = []
    ...
    def parse_implicit_document_start(self):
        if not self.scanner.check_token(
            ...
            event = DocumentStartEvent(start_mark, end_mark, explicit=_B)
    ...
    def parse_document_start(self):
        ...
        if not self.scanner.check_token(StreamEndToken):
            ...
            event = DocumentStartEvent(
                start_mark, end_mark, explicit=_C, version=version, tags=tags
            )
            self.states.append(self.parse_document_end)
    ...
    def process_directives(self):
        ...
        while self.scanner.check_token(DirectiveToken):
            token = self.scanner.get_token()
            if token.name == "YAML":
                if yaml_version is not _A:
                    raise ParserError(
                        _A, _A, "found duplicate YAML directive", token.start_mark
                    )
    ...
    def parse_block_node(self):
        return self.parse_node(block=_C)

    def parse_flow_node(self):
        return self.parse_node()

    def parse_block_node_or_indentless_sequence(self):
        return self.parse_node(block=_C, indentless_sequence=_C)
    ...
    def parse_node(self, block=_B, indentless_sequence=_B):
        ...
        anchor = _A
        tag = _A
        start_mark = end_mark = tag_mark = _A
        ...
        if self.scanner.check_token(ScalarToken):
            token = self.scanner.get_token()
            end_mark = token.end_mark
            if token.plain and tag is _A or tag == _D:
                implicit = _C, _B
            elif tag is _A:
                implicit = _B, _C
            else:
            ...
            pt = self.scanner.peek_token()
            comment = pt.comment
            if comment is _A or comment[1] is _A:
        ...
        elif anchor is not _A or tag is not _A:
            event = ScalarEvent(anchor, tag, (implicit, _B), "", start_mark, end_mark)
            self.state = self.states.pop()
        else:
            if block:
                node = "block"
    ...
    def parse_block_mapping_value(self):
        if self.scanner.check_token(ValueToken):
            ...
            if self.scanner.check_token(ValueToken):
                ...
                token.move_comment(self.scanner.peek_token(), empty=_C)
            if not self.scanner.check_token(KeyToken, ValueToken, BlockEndToken):
                ...
                if comment is _A:
                    ...
                    comment = token.comment
                    if comment:
                        token._comment = [_A, comment[1]]
                        comment = [comment[0], _A]
    ...
    def parse_flow_sequence_first_entry(self):
        token = self.scanner.get_token()
        self.marks.append(token.start_mark)
        return self.parse_flow_sequence_entry(first=_C)
    ...
    def parse_flow_sequence_entry(self, first=_B):
        if not self.scanner.check_token(FlowSequenceEndToken):
            ...
            if self.scanner.check_token(KeyToken):
                token = self.scanner.peek_token()
                event = MappingStartEvent(
                    _A, _A, _C, token.start_mark, token.end_mark, flow_style=_C
                )
    ...
    def parse_flow_mapping_first_key(self):
        token = self.scanner.get_token()
        self.marks.append(token.start_mark)
        return self.parse_flow_mapping_key(first=_C)
    ...
    def process_empty_scalar(self, mark, comment=_A):
        return ScalarEvent(_A, _A, (_C, _B), "", mark, mark, comment=comment)
