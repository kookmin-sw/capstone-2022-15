...
if _B:
    from typing import Any, Dict, List, Union, Text, Optional
...
implicit_resolvers = [
    ...
    ([(1, 2), (1, 1)], "tag:yaml.org,2002:merge", RegExp("^(?:<<)$"), ["<"]),
    (
        [(1, 2), (1, 1)],
    ...
    ([(1, 2), (1, 1)], "tag:yaml.org,2002:value", RegExp("^(?:=)$"), ["="]),
    ([(1, 2), (1, 1)], "tag:yaml.org,2002:yaml", RegExp("^(?:!|&|\\*)$"), list("!&*")),
]
...
class BaseResolver:
    ...
    def __init__(self, loadumper=_A):
        ...
        if self.loadumper is not _A and getattr(self.loadumper, "_resolver", _A) is _A:
            self.loadumper._resolver = self.loadumper
        self._loader_version = _A
    ...
    def add_path_resolver(cls, tag, path, kind=_A):
        ...
        if kind is str:
            ...
            kind = MappingNode
        elif kind not in [ScalarNode, SequenceNode, MappingNode] and kind is not _A:
    ...
    def check_resolver_prefix(self, depth, path, kind, current_node, current_index):
        ...
        if isinstance(node_check, string_types):
            ...
            if not isinstance(current_node, node_check):
                return _B
        if index_check is _C and current_index is not _A:
            return _B
        if (index_check is _B or index_check is _A) and current_index is _A:
            return _B
    ...
    def resolve(self, kind, value, implicit):
        ...
        if bool(self.yaml_path_resolvers):
            ...
            if _A in exact_paths:
                return exact_paths[_A]
...
class VersionedResolver(BaseResolver):
    def __init__(self, version=_A, loader=_A, loadumper=_A):
        if loader is _A and loadumper is not _A:
            loader = loadumper
    ...
    def get_loader_version(self, version):
        if version is _A or isinstance(version, tuple):
            return version
