...
def hidden_prompt_func(prompt):
    ...
    return A.getpass(prompt)


def _build_prompt(text, suffix, show_default=_B, default=_A, show_choices=_C, type=_A):
    B = default
    A = text
    if type is not _A and show_choices and isinstance(type, Choice):
        A += f" ({', '.join(map(str,type.choices))})"
    if B is not _A and show_default:
        A = f"{A} [{_format_default(B)}]"
    return f"{A}{suffix}"


def _format_default(default):
    A = default
    if isinstance(A, (io.IOBase, LazyFile)) and hasattr(A, "name"):
        return A.name
    return A
...
def prompt(
    ...
    while 1:
        ...
        try:
            E = B(D)
        except UsageError as J:
            echo(f"Error: {J.message}", err=C)
            continue
...
def confirm(text, default=_B, abort=_B, prompt_suffix=": ", show_default=_C, err=_B):
    C = default
    D = _build_prompt(text, prompt_suffix, show_default, "Y/n" if C else "y/N")
    while 1:
        try:
            echo(D, nl=_B, err=err)
            B = visible_prompt_func("").lower().strip()
        except (KeyboardInterrupt, EOFError):
            raise Abort()
        if B in ("y", "yes"):
            A = _C
...
def get_terminal_size():
    ...
    if hasattr(C, "get_terminal_size"):
        return C.get_terminal_size()
    ...
    def B(fd):
        try:
            import fcntl, termios as A

            B = struct.unpack("hh", fcntl.ioctl(fd, A.TIOCGWINSZ, "1234"))
        except Exception:
            return
        return B
    ...
    if not A or not A[0] or not A[1]:
        A = os.environ.get("LINES", 25), os.environ.get("COLUMNS", DEFAULT_COLUMNS)
...
def echo_via_pager(text_or_generator, color=_A):
    ...
    if inspect.isgeneratorfunction(A):
        C = A()
...
def progressbar(
    ...
    A = color
    from ._termui_impl import ProgressBar as B

    A = resolve_color_default(A)
    return B(
        iterable=iterable,
        length=length,
        show_eta=show_eta,
...
def style(
    text, fg=_A, bg=_A, bold=_A, dim=_A, underline=_A, blink=_A, reverse=_A, reset=_C
):
    D = reverse
    C = blink
    B = underline
    ...
    if fg:
        try:
            A.append(f"[{_ansi_colors[fg]}m")
        except KeyError:
            raise TypeError(f"Unknown color {fg!r}")
    if bg:
        try:
            A.append(f"[{_ansi_colors[bg]+10}m")
        except KeyError:
            raise TypeError(f"Unknown color {bg!r}")
    if bold is not _A:
        A.append(f"[{1 if bold else 22}m")
    if dim is not _A:
        A.append(f"[{2 if dim else 22}m")
    if B is not _A:
        A.append(f"[{4 if B else 24}m")
    if C is not _A:
        A.append(f"[{5 if C else 25}m")
    if D is not _A:
        A.append(f"[{7 if D else 27}m")
    A.append(text)
    if reset:
        A.append(_ansi_reset_all)
    return "".join(A)
...
def edit(text=_A, editor=_A, env=_A, require_save=_C, extension=".txt", filename=_A):
    B = filename
    A = editor
    from ._termui_impl import Editor as C

    A = C(editor=A, env=env, require_save=require_save, extension=extension)
    if B is _A:
        return A.edit(text)
    A.edit_file(B)
...
def getchar(echo=_B):
    A = _getchar
    if A is _A:
        from ._termui_impl import getchar as A
    return A(echo)
