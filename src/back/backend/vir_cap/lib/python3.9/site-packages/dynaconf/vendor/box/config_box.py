...
class ConfigBox(Box):
    ...
    def __dir__(A):
        return super().__dir__() + [_B, _C, _D, _E, _F, _G, _H]
    ...
    def bool(C, item, default=_A):
        ...
        if isinstance(A, (bool, int)):
            return bool(A)
        if isinstance(A, str) and A.lower() in ("n", "no", "false", "f", "0"):
            return E
    ...
    def list(E, item, default=_A, spliter=",", strip=True, mod=_A):
        ...
        if mod:
            return list(map(mod, D))
        return D

    def getboolean(A, item, default=_A):
        return A.bool(item, default)

    def getint(A, item, default=_A):
        return A.int(item, default)

    def getfloat(A, item, default=_A):
        return A.float(item, default)

    def __repr__(A):
        return "<ConfigBox: {0}>".format(str(A.to_dict()))
