...
try:
    from setuptools.namespaces import Installer as NameSpaceInstaller
except ImportError:
...
if os.environ.get(_M, "") == "":
    ...
    def debug(*args, **kw):
        with open(os.environ[_M], "a") as fp:
            kw1 = kw.copy()
            kw1["file"] = fp
            print(
                "{:%Y-%d-%mT%H:%M:%S}".format(datetime.datetime.now()), file=fp, end=" "
            )
            print(*args, **kw1)
                    ...
                    try:
                        ...
                        for (index, line) in enumerate(lines):
                            if from_line <= index <= to_line:
                                print(
                                    "{0:{1}}: {2}".format(index, w, line).encode(_E),
                                    end="",
                                )
                                if index == e.lineno - 1:
                                    print(
...
class MySdist(_sdist):
    def initialize_options(self):
        ...
        dist_base = os.environ.get(_O)
        fpn = getattr(getattr(self, _P, self), _I, _A)
        if fpn and dist_base:
            print(_Q.format(dist_base, fpn))
...
class NameSpacePackager:
    def __init__(self, pkg_data):
        ...
        self.full_package_name = self.pn(self._pkg_data[_I])
        self._split = _A
        self.depth = self.full_package_name.count(_B)
        self.nested = self._pkg_data.get(_R, _C)
        if self.nested:
            NameSpaceInstaller.install_namespaces = lambda x: _A
        self.command = _A
        self.python_version()
        self._pkg = [_A, _A]
        if (
            ...
            if os.environ.get("READTHEDOCS", _A) == _N:
                os.system("pip install .")
                sys.exit(0)
            if not os.environ.get("RUAMEL_NO_PIP_INSTALL_CHECK", _C):
                print('error: you have to install with "pip install ."')
                sys.exit(1)
        if self._pkg_data.get(_S):
            Distribution.is_pure = lambda *args: _D
        else:
            Distribution.is_pure = lambda *args: _C
        for x in sys.argv:
            if x[0] == "-" or x == _F:
                continue
            self.command = x
    ...
    def split(self):
        ...
        if self._split is _A:
            fpn = self.full_package_name.split(_B)
            self._split = []
            while fpn:
                self._split.insert(0, _B.join(fpn))
                fpn = fpn[:-1]
            ...
            if sys.version_info < (3,):
                self._split = [
                    y.encode(_E) if isinstance(y, unicode) else y for y in self._split
                ]
        if skip:
            0
        return self._split
    ...
    def create_dirs(self):
        ...
        if not os.path.exists(directories[0]):
            for d in directories:
                os.mkdir(d)
                with open(os.path.join(d, _H), "w") as fp:
                    fp.write(
    ...
    def check(self):
        ...
        for p in sys.path:
            if not p:
                continue
            if os.path.exists(os.path.join(p, _F)):
                continue
            ...
            for fn in os.listdir(p):
                ...
                if fn == prefix and os.path.isdir(full_name):
                    if self.command == A:
                        raise InstallationError(
                            "Cannot mix develop (pip install -e),\nwith non-develop installs for package name {0}".format(
                                fn
                            )
                        )
                elif fn == prefix:
                    raise InstallationError(
                        "non directory package {0} in {1}".format(fn, p)
                    )
                for pre in [x + _B for x in prefixes]:
                    ...
                    continue
                if fn.endswith("-link") and self.command == _J:
                    raise InstallationError(
                        "Cannot mix non-develop with develop\n(pip install -e) installs for package name {0}".format(
                            fn
                        )
                    )
    ...
    def entry_points(self, script_name=_A, package_name=_A):
        ...
        if ep not in [_D, 1]:
            if "=" in ep:
                return {A: [ep]}
            script_name = ep
        if package_name is _A:
            package_name = self.full_package_name
        if not script_name:
            script_name = package_name.split(_B)[-1]
    ...
    def status(self):
        A = "β"
        status = self._pkg_data.get("status", A).lower()
        if status in ["α", "alpha"]:
            return 3, "Alpha"
        elif status in [A, "beta"]:
            return 4, "Beta"
    ...
    def package_data(self):
        ...
        if self.has_mit_lic():
            df.append(_L)
            exclude_files.append(_L)
        if self._pkg_data.get("binary_only", _C):
            exclude_files.append(_H)
        debug("testing<<<<<")
    ...
    def packages(self):
        s = self.split
        return s + self._pkg_data.get(_T, [])

    @property
    def python_requires(self):
        return self._pkg_data.get("python_requires", _A)
    ...
    def ext_modules(self):
        ...
        if _U in sys.argv:
            return _A
        if platform.python_implementation() == _K:
            return _A
        try:
        ...
        if no_test_compile:
            for target in self._pkg_data.get(C, []):
                ext = Extension(
                    self.pn(target[A]),
                    sources=[self.pn(x) for x in target[D]],
                    libraries=[self.pn(x) for x in target.get(E)],
                )
        ...
        for target in self._pkg_data.get(C, []):
            ...
            try:
                ...
                bin_file_name = B + self.pn(target[A])
...
def main():
    ...
    if dump_kw in sys.argv:
        import wheel, distutils, setuptools
    ...
    kw = dict(
        ...
        extras_require=nsp.extras_require,
        license=nsp.license,
    ...
    for x in ["-c", "egg_info", "--egg-base", "pip-egg-info"]:
        ...
        for p in nsp.install_pre:
            ...
            setup_path = os.path.join(*p.split(_B) + [_F])
            ...
            while len(try_dir) > 1:
                ...
                if os.path.exists(full_path_setup_py):
                    pip = sys.executable.replace(_G, "pip")
                    cmd = [pip, _J, os.path.dirname(full_path_setup_py)]
