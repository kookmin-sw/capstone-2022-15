...
def unset(ctx, key):
    ...
    D, A = unset_key(B, A, C)
    if D:
        click.echo("Successfully removed %s" % A)
    else:
        exit(1)
...
def run(ctx, commandline):
    A = commandline
    B = ctx.obj[_B]
    C = {to_env(C): to_env(A) for (C, A) in dotenv_values(B).items() if A is not None}
    if not A:
...
def run_command(command, env):
    A = os.environ.copy()
    A.update(env)
    B = Popen(command, universal_newlines=_A, bufsize=0, shell=False, env=A)
    C, C = B.communicate()
    return B.returncode
