...
def box_from_file(file, file_type=None, encoding="utf-8", errors="strict"):
    ...
    if C:
        ...
        if C.lower() == "toml":
            return _to_toml(B)
        raise BoxError(
            f'"{C}" is an unknown type, please use either toml, yaml or json'
        )
    if A.suffix in (".json", ".jsn"):
        return _to_json(B)
    if A.suffix in (".yaml", ".yml"):
        return _to_yaml(B)
    if A.suffix in (".tml", ".toml"):
        return _to_toml(B)
    raise BoxError(
        f"Could not determine file type based off extension, please provide file_type"
    )
