...
if _B:
    from typing import Any, Dict, Optional, List, Union, Text
...
class Scanner:
    def __init__(self, loader=_C):
        self.loader = loader
        if self.loader is not _C and getattr(self.loader, "_scanner", _C) is _C:
            self.loader._scanner = self
        self.reset_scanner()
        self.first_time = _B
        self.yaml_version = _C
    ...
    def fetch_more_tokens(self):
        ...
        if ch == _N and self.check_document_start():
        ...
        if ch == _N and self.check_block_entry():
            return self.fetch_block_entry()
        if ch == "?" and self.check_key():
            return self.fetch_key()
        if ch == _O and self.check_value():
        ...
        if ch == _R and not self.flow_level:
    ...
    def next_possible_simple_key(self):
        ...
        for level in self.possible_simple_keys:
            ...
            if min_token_number is _C or key.token_number < min_token_number:
    ...
    def stale_possible_simple_keys(self):
        for level in list(self.possible_simple_keys):
            ...
            if key.line != self.reader.line or self.reader.index - key.index > 1024:
                if key.required:
                    raise ScannerError(_d, key.mark, _e, self.reader.get_mark())
    ...
    def fetch_key(self):
        if not self.flow_level:
            if not self.allow_simple_key:
                raise ScannerError(
                    _C, _C, "mapping keys are not allowed here", self.reader.get_mark()
                )
            if self.add_indent(self.reader.column):
                mark = self.reader.get_mark()
    ...
    def fetch_folded(self):
        self.fetch_block_scalar(style=_R)
    ...
    def fetch_single(self):
        self.fetch_flow_scalar(style=_J)

    def fetch_double(self):
        self.fetch_flow_scalar(style=_P)
    ...
    def check_plain(self):
        ...
        if self.scanner_processing_version == (1, 1):
            return (
                ch not in A
                or srp(1) not in _THE_END_SPACE_TAB
                and (ch == _N or not self.flow_level and ch in B)
            )
        if ch not in A:
            return _A
        ch1 = srp(1)
        if ch == _N and ch1 not in _THE_END_SPACE_TAB:
            return _A
        if ch == _O and bool(self.flow_level) and ch1 not in _SPACE_TAB:
            return _A
        return srp(1) not in _THE_END_SPACE_TAB and (
            ch == _N or not self.flow_level and ch in B
        )

    def scan_to_next_token(self):
        srp = self.reader.peek
        srf = self.reader.forward
        if self.reader.index == 0 and srp() == _f:
            srf()
        found = _B
        _the_end = _THE_END
        while not found:
    ...
    def scan_directive_name(self, start_mark):
        ...
        srp = self.reader.peek
        ch = srp(length)
        while _K <= ch <= _Q or "A" <= ch <= "Z" or _U <= ch <= "z" or ch in "-_:.":
            length += 1
            ch = srp(length)
        if not length:
            raise ScannerError(_G, start_mark, _V % utf8(ch), self.reader.get_mark())
    ...
    def scan_tag_directive_handle(self, start_mark):
        value = self.scan_tag_handle(_g, start_mark)
        ch = self.reader.peek()
        if ch != _D:
            raise ScannerError(_G, start_mark, _Z % utf8(ch), self.reader.get_mark())
        return value

    def scan_tag_directive_prefix(self, start_mark):
        value = self.scan_tag_uri(_g, start_mark)
        ch = self.reader.peek()
        if ch not in _L:
            raise ScannerError(_G, start_mark, _Z % utf8(ch), self.reader.get_mark())
        return value
    ...
    def scan_directive_ignored_line(self, start_mark):
        ...
        if ch not in _THE_END:
            raise ScannerError(_G, start_mark, _h % utf8(ch), self.reader.get_mark())
        self.scan_line_break()

    def scan_anchor(self, TokenClass):
        A = "while scanning an %s"
        srp = self.reader.peek
        start_mark = self.reader.get_mark()
        indicator = srp()
        if indicator == "*":
        ...
        if not length:
            raise ScannerError(
                A % (name,), start_mark, _V % utf8(ch), self.reader.get_mark()
            )
        value = self.reader.prefix(length)
        self.reader.forward(length)
        if ch not in "\x00 \t\r\n\x85\u2028\u2029?:,[]{}%@`":
    ...
    def scan_tag(self):
        ...
        if ch == "<":
            ...
            handle = _H
            if use_handle:
                handle = self.scan_tag_handle(A, start_mark)
            else:
                handle = _H
                self.reader.forward()
            suffix = self.scan_tag_uri(A, start_mark)
    ...
    def scan_block_scalar(self, style, rt=_B):
        ...
        if increment is _C:
            if min_indent < 1 and (
                ...
                and getattr(
                    self.loader, "top_level_block_style_scalar_no_indent_error_1_1", _B
                )
            ):
                min_indent = 1
        ...
        while self.reader.column == indent and srp() != _I:
            ...
            leading_non_space = srp() not in _M
            length = 0
            ...
            if self.reader.column == indent and srp() != _I:
                if rt and folded and line_break == _E:
                    chunks.append(_i)
                if (
                    folded
                    and line_break == _E
    ...
    def scan_block_scalar_indicators(self, start_mark):
        D = "expected indentation indicator in the range 1-9, but found 0"
        C = "0123456789"
        B = "+"
        A = "+-"
        srp = self.reader.peek
        ...
        if ch in A:
            ...
            if ch in C:
                increment = int(ch)
                if increment == 0:
                    raise ScannerError(_W, start_mark, D, self.reader.get_mark())
                self.reader.forward()
        elif ch in C:
            increment = int(ch)
            if increment == 0:
    ...
    def scan_block_scalar_ignored_line(self, start_mark):
        ...
        ch = srp()
        if ch not in _THE_END:
            raise ScannerError(_W, start_mark, _h % utf8(ch), self.reader.get_mark())
        self.scan_line_break()
        return comment
    ...
    def scan_block_scalar_breaks(self, indent):
        ...
        srf = self.reader.forward
        end_mark = self.reader.get_mark()
        while self.reader.column < indent and srp() == _D:
            srf()
        while srp() in _X:
            chunks.append(self.scan_line_break())
            end_mark = self.reader.get_mark()
    ...
    def scan_flow_scalar_non_spaces(self, double, start_mark):
        A = "while scanning a double-quoted scalar"
        chunks = []
        srp = self.reader.peek
        srf = self.reader.forward
        while _A:
            ...
            if length != 0:
                ...
                srf(length)
            ch = srp()
            if not double and ch == _J and srp(1) == _J:
                chunks.append(_J)
                srf(2)
            elif double and ch == _J or not double and ch in '"\\':
                chunks.append(ch)
                srf()
    ...
    def scan_flow_scalar_spaces(self, double, start_mark):
        ...
        if ch == _I:
            raise ScannerError(
                _l, start_mark, "found unexpected end of stream", self.reader.get_mark()
            )
        elif ch in _X:
            line_break = self.scan_line_break()
    ...
    def scan_flow_scalar_breaks(self, double, start_mark):
        ...
        while _A:
            ...
            if (prefix == _S or prefix == _T) and srp(3) in _THE_END_SPACE_TAB:
                raise ScannerError(
                    _l,
                    start_mark,
    ...
    def scan_plain(self):
        ...
        while _A:
            ...
            while _A:
                ch = srp(length)
                if ch == _O and srp(length + 1) not in _THE_END_SPACE_TAB:
                    0
            ...
            if (
                ...
                or srp() == _F
                or not self.flow_level
                and self.reader.column < indent
            ):
                break
        token = ScalarToken("".join(chunks), _A, start_mark, end_mark)
        if spaces and spaces[0] == _E:
            comment = CommentToken("".join(spaces) + _E, start_mark, end_mark)
            token.add_post_comment(comment)
        return token
    ...
    def scan_tag_handle(self, name, start_mark):
        A = "expected '!', but found %r"
        srp = self.reader.peek
        ch = srp()
        if ch != _H:
            raise ScannerError(
                _Y % (name,), start_mark, A % utf8(ch), self.reader.get_mark()
            )
        length = 1
        ch = srp(length)
        if ch != _D:
    ...
    def scan_uri_escapes(self, name, start_mark):
        ...
        try:
            if PY3:
                value = bytes(code_bytes).decode(A)
            else:
                value = unicode(b"".join(code_bytes), A)
        except UnicodeDecodeError as exc:
            raise ScannerError(_Y % (name,), start_mark, str(exc), mark)
        return value

    def scan_line_break(self):
        ch = self.reader.peek()
        if ch in _m:
            if self.reader.prefix(2) == _n:
                self.reader.forward(2)
            else:
                self.reader.forward()
            return _E
...
class RoundTripScanner(Scanner):
        ...
        if bool(self.tokens):
            if (
                ...
                while len(self.tokens) > 1 and isinstance(self.tokens[1], CommentToken):
                    ...
                    c.value = c.value + _D * c1.start_mark.column + c1.value
                    self.fetch_more_tokens()
                ...
                c = self.tokens.pop(1)
                c.value = (
                    _E * (c.start_mark.line - self.tokens[0].end_mark.line)
                    + _D * c.start_mark.column
                    + c.value
                )
                self.tokens[0].add_post_comment(c)
                self.fetch_more_tokens()
        ...
        while not found:
            ...
            if bool(self.scan_line_break()):
                ...
                if ch == _E:
                    ...
                    comment = ""
                    while ch:
                        ch = self.scan_line_break(empty_line=_A)
                        comment += ch
                    if srp() == _F:
                        comment = comment.rsplit(_E, 1)[0] + _E
                    end_mark = self.reader.get_mark()
                    return comment, start_mark, end_mark
            else:
                found = _A
    ...
    def scan_block_scalar(self, style, rt=_A):
