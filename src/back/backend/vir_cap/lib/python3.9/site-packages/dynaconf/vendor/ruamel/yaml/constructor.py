...
if _B:
    from typing import Any, Dict, List, Set, Generator, Union, Optional
__all__ = [
    "BaseConstructor",
    "SafeConstructor",
    "Constructor",
...
class BaseConstructor:
    ...
    def __init__(self, preserve_quotes=_A, loader=_A):
        self.loader = loader
        if self.loader is not _A and getattr(self.loader, "_constructor", _A) is _A:
    ...
    def construct_object(self, node, deep=_B):
        ...
        if deep:
            ...
            self.deep_construct = _C
        if node in self.recursive_objects:
            return self.recursive_objects[node]
        self.recursive_objects[node] = _A
    ...
    def construct_non_recursive_object(self, node, tag=_A):
        ...
        if tag in self.yaml_constructors:
            ...
            for tag_prefix in self.yaml_multi_constructors:
                ...
                if _A in self.yaml_multi_constructors:
                    tag_suffix = tag
                    constructor = self.yaml_multi_constructors[_A]
                elif _A in self.yaml_constructors:
                    constructor = self.yaml_constructors[_A]
    ...
    def construct_scalar(self, node):
        if not isinstance(node, ScalarNode):
            raise ConstructorError(_A, _A, _g % node.id, node.start_mark)
        return node.value

    def construct_sequence(self, node, deep=_B):
        if not isinstance(node, SequenceNode):
            raise ConstructorError(_A, _A, _h % node.id, node.start_mark)
        return [self.construct_object(child, deep=deep) for child in node.value]

    def construct_mapping(self, node, deep=_B):
        if not isinstance(node, MappingNode):
            raise ConstructorError(_A, _A, _P % node.id, node.start_mark)
        total_mapping = self.yaml_base_dict_type()
        if getattr(node, "merge", _A) is not _A:
            todo = [(node.merge, _B), (node.value, _B)]
        else:
            todo = [(node.value, _C)]
                ...
                if PY2:
                    try:
                        ...
                        raise ConstructorError(
                            _E, node.start_mark, _W % exc, key_node.start_mark
                        )
                elif not isinstance(key, Hashable):
                    raise ConstructorError(_E, node.start_mark, _X, key_node.start_mark)
            ...
            if not self.allow_duplicate_keys:
                ...
                if PY2:
                    ...
                    if isinstance(value, unicode):
                        value = value.encode(_Q)
                    if isinstance(mk, unicode):
                        mk = mk.encode(_Q)
                args = [
                    _E,
                    node.start_mark,
                    'found duplicate key "{}" with value "{}" (original value: "{}")'.format(
                        key, value, mk
                    ),
                    key_node.start_mark,
                    _i,
                ...
                if self.allow_duplicate_keys is _A:
...
class SafeConstructor(BaseConstructor):
                ...
                if isinstance(value_node, MappingNode):
                    ...
                    for subnode in value_node.value:
                        if not isinstance(subnode, MappingNode):
                            raise ConstructorError(
                                _E, node.start_mark, _n % subnode.id, subnode.start_mark
                            )
                    ...
                    raise ConstructorError(
                        _E, node.start_mark, _o % value_node.id, value_node.start_mark
                    )
            elif key_node.tag == _Z:
                key_node.tag = _R
                index += 1
            else:
                index += 1
    ...
    def construct_yaml_int(self, node):
        ...
        if value_s == _F:
            ...
            digits = [int(part) for part in value_s.split(_G)]
            digits.reverse()
            base = 1
            value = 0
            for digit in digits:
    ...
    def construct_yaml_float(self, node):
        ...
        value_s = value_so.replace(_D, "").lower()
        sign = +1
        if value_s[0] == _J:
            sign = -1
        if value_s[0] in _M:
        ...
        if value_s == _p:
            ...
            digits = [float(part) for part in value_s.split(_G)]
            digits.reverse()
            base = 1
            value = 0.0
            for digit in digits:
    ...
    if PY3:
        ...
        def construct_yaml_binary(self, node):
            try:
                value = self.construct_scalar(node).encode(_S)
            except UnicodeEncodeError as exc:
                raise ConstructorError(_A, _A, _r % exc, node.start_mark)
            try:
                if hasattr(base64, _s):
                    return base64.decodebytes(value)
                else:
                    return base64.decodestring(value)
            except binascii.Error as exc:
                raise ConstructorError(_A, _A, _a % exc, node.start_mark)
    ...
    def construct_yaml_timestamp(self, node, values=_A):
        if values is _A:
            try:
                ...
                match = _A
            if match is _A:
                raise ConstructorError(_A, _A, _t.format(node.value), node.start_mark)
        ...
        hour = int(values[_T])
        minute = int(values[_x])
        second = int(values[_y])
        fraction = 0
        if values[_I]:
            ...
            if len(values[_I]) > 6 and int(values[_I][6]) > 4:
                fraction += 1
        delta = _A
        if values[_K]:
            tz_hour = int(values[_U])
            minutes = values[_O]
    ...
    def construct_yaml_omap(self, node):
        ...
        yield omap
        if not isinstance(node, SequenceNode):
            raise ConstructorError(_L, node.start_mark, _b % node.id, node.start_mark)
        for subnode in node.value:
            if not isinstance(subnode, MappingNode):
                raise ConstructorError(
                    _L, node.start_mark, _c % subnode.id, subnode.start_mark
                )
            if len(subnode.value) != 1:
                raise ConstructorError(
                    _L, node.start_mark, _d % len(subnode.value), subnode.start_mark
                )
    ...
    def construct_yaml_pairs(self, node):
        A = "while constructing pairs"
        pairs = []
        yield pairs
        if not isinstance(node, SequenceNode):
            raise ConstructorError(A, node.start_mark, _b % node.id, node.start_mark)
        for subnode in node.value:
            if not isinstance(subnode, MappingNode):
                raise ConstructorError(
                    A, node.start_mark, _c % subnode.id, subnode.start_mark
                )
            if len(subnode.value) != 1:
                raise ConstructorError(
                    A, node.start_mark, _d % len(subnode.value), subnode.start_mark
                )
            key_node, value_node = subnode.value[0]
            key = self.construct_object(key_node)
            value = self.construct_object(value_node)
    ...
    def construct_yaml_object(self, node, cls):
        ...
        yield data
        if hasattr(data, _V):
            state = self.construct_mapping(node, deep=_C)
...
class Constructor(SafeConstructor):
    ...
    def find_python_module(self, name, mark):
        if not name:
            raise ConstructorError(_e, mark, _AB, mark)
        try:
            __import__(name)
        except ImportError as exc:
            raise ConstructorError(_e, mark, _AC % (utf8(name), exc), mark)
        return sys.modules[name]

    def find_python_name(self, name, mark):
        A = "while constructing a Python object"
        if not name:
            raise ConstructorError(A, mark, _AB, mark)
        if _H in name:
            lname = name.split(_H)
            lmodule_name = lname
            lobject_name = []
            while len(lmodule_name) > 1:
                ...
                module_name = _H.join(lmodule_name)
                try:
        ...
        try:
            ...
            raise ConstructorError(A, mark, _AC % (utf8(module_name), exc), mark)
        module = sys.modules[module_name]
        object_name = _H.join(lobject_name)
        obj = module
        while lobject_name:
    ...
    def construct_python_module(self, suffix, node):
        ...
        if value:
            raise ConstructorError(
                _e, node.start_mark, _AD % utf8(value), node.start_mark
            )
    ...
    def construct_python_object(self, suffix, node):
        instance = self.make_python_instance(suffix, node, newobj=_C)
        self.recursive_objects[node] = instance
        yield instance
        deep = hasattr(instance, _V)
        state = self.construct_mapping(node, deep=deep)
        self.set_python_instance_state(instance, state)

    def construct_python_object_apply(self, suffix, node, newobj=_B):
        if isinstance(node, SequenceNode):
            args = self.construct_sequence(node, deep=_C)
            kwds = {}
            state = {}
            listitems = []
            dictitems = {}
        else:
            value = self.construct_mapping(node, deep=_C)
            args = value.get("args", [])
            kwds = value.get("kwds", {})
    ...
    def construct_python_object_new(self, suffix, node):
        return self.construct_python_object_apply(suffix, node, newobj=_C)
...
class RoundTripConstructor(SafeConstructor):
    ...
    def construct_yaml_int(self, node):
        ...
        try:
            sx = value_su.rstrip(_D)
            underscore = [len(sx) - sx.rindex(_D) - 1, _B, _B]
        except ValueError:
            underscore = _A
        except IndexError:
            underscore = _A
        value_s = value_su.replace(_D, "")
        sign = +1
        if value_s[0] == _J:
            sign = -1
        if value_s[0] in _M:
        ...
        if value_s == _F:
            ...
            if self.resolver.processing_version > (1, 1) and value_s[2] == _F:
                width = len(value_s[2:])
            if underscore is not _A:
                underscore[1] = value_su[2] == _D
                underscore[2] = len(value_su[2:]) > 1 and value_su[-1] == _D
            return BinaryInt(
        ...
        elif self.resolver.processing_version != (1, 2) and value_s[0] == _F:
            return sign * int(value_s, 8)
        elif self.resolver.processing_version != (1, 2) and _G in value_s:
            digits = [int(part) for part in value_s.split(_G)]
            digits.reverse()
            ...
            if underscore is not _A:
                underscore[2] = len(value_su) > 1 and value_su[-1] == _D
            return ScalarInt(
            ...
            underscore[2] = len(value_su) > 1 and value_su[-1] == _D
            return ScalarInt(
                sign * int(value_s), width=_A, underscore=underscore, anchor=node.anchor
            )
        elif node.anchor:
            return ScalarInt(sign * int(value_s), width=_A, anchor=node.anchor)
        else:
            return sign * int(value_s)
    ...
    def construct_yaml_float(self, node):
        ...
        if _N in value_s:
            try:
                mantissa, exponent = value_so.split(_N)
                exp = _N
            except ValueError:
                mantissa, exponent = value_so.split(A)
                exp = A
            ...
            width = len(mantissa)
            prec = mantissa.find(_H)
            if m_sign:
                width -= 1
            e_width = len(exponent)
            e_sign = exponent[0] in _M
            return ScalarFloat(
                sign * float(value_s),
                width=width,
                prec=prec,
    ...
    def construct_rt_sequence(self, node, seqtyp, deep=_B):
        ...
        if node.comment:
            ...
            if len(node.comment) > 2:
                seqtyp.yaml_end_comment_extend(node.comment[2], clear=_C)
        if node.anchor:
    ...
    def flatten_mapping(self, node):
        def constructed(value_node):
            if value_node in self.constructed_objects:
                ...
                value = self.construct_object(value_node, deep=_B)
            return value
    ...
    def construct_mapping(self, node, maptyp, deep=_B):
        ...
        if node.comment:
            ...
            if len(node.comment) > 2:
                maptyp.yaml_end_comment_extend(node.comment[2], clear=_C)
        if node.anchor:
            from dynaconf.vendor.ruamel.yaml.serializer import templated_id
        ...
        for (key_node, value_node) in node.value:
            ...
            if not isinstance(key, Hashable):
                if isinstance(key, MutableSequence):
                    ...
                    if key_node.flow_style is _C:
                        key_s.fa.set_flow_style()
                    elif key_node.flow_style is _B:
    ...
    def construct_setting(self, node, typ, deep=_B):
        ...
        if node.comment:
            ...
            if len(node.comment) > 2:
                typ.yaml_end_comment_extend(node.comment[2], clear=_C)
        if node.anchor:
            from dynaconf.vendor.ruamel.yaml.serializer import templated_id
    ...
    def construct_yaml_map(self, node):
        ...
        data._yaml_set_line_col(node.start_mark.line, node.start_mark.column)
        yield data
        self.construct_mapping(node, data, deep=_C)
        self.set_collection_style(data, node)

    def set_collection_style(self, data, node):
        if len(data) == 0:
            return
        if node.flow_style is _C:
            data.fa.set_flow_style()
        elif node.flow_style is _B:
    ...
    def construct_yaml_object(self, node, cls):
        ...
        yield data
        if hasattr(data, _V):
            state = SafeConstructor.construct_mapping(self, node, deep=_C)
            data.__setstate__(state)
        else:
    ...
    def construct_yaml_omap(self, node):
        ...
        if node.comment:
            ...
            if len(node.comment) > 2:
                omap.yaml_end_comment_extend(node.comment[2], clear=_C)
        if not isinstance(node, SequenceNode):
            raise ConstructorError(_L, node.start_mark, _b % node.id, node.start_mark)
        for subnode in node.value:
            if not isinstance(subnode, MappingNode):
    ...
    def construct_undefined(self, node):
        try:
            if isinstance(node, MappingNode):
                ...
                yield data2
                if node.anchor:
                    data2.yaml_set_anchor(node.anchor, always_dump=_C)
                return
            elif isinstance(node, SequenceNode):
                data3 = CommentedSeq()
