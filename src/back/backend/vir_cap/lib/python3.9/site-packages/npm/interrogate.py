...
from .exceptions import (
    ErrorInterrogatingEnvironment,
    MalformedVersionInput,
    MissingDependency,
    OutdatedDependency,
)
...
def _check_if_version_is_outdated(current_version, required_version):
    ...
    if len(required_version) < 3:
        raise MalformedVersionInput(
            "Versions must have three numbers defined. Received {0}".format(
                required_version
            )
        )
    for number in required_version:
        if not isinstance(number, six.integer_types):
            raise MalformedVersionInput(
                "Versions can only contain number. Received {0}".format(
                    required_version
                )
            )
...
def raise_if_npm_missing(required_version=None):
    if not npm_installed:
        error = 'npm is not installed or cannot be found at path "{path}"'.format(
            path=settings.PATH
        )
        if required_version:
